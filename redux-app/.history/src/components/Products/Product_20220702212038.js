...
function Product() {
  ...
  const getTheSingleProduct = async()=>{
    const {data2} = await axios.get(`https://dummyapi.io/data/v1/user/${id}`);
        ...
        Object.keys(users1).length>0 ? (
            <div className='row border border-primary mt-2'>
                <div className='col-md-6' >
                    <img className= 'img-fluid' src={users1.image} alt={users1.title} />
                    </div>
                    ...
                    <div className='col-md-6'>
                      ...
                      <div className='product_price'>
                       <span className='bg-info p-2 text-white'>Price: ${users1.price}</span>
                      </div>
